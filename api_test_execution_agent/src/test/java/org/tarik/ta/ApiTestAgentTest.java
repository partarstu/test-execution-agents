...
class ApiTestAgentTest {
    ...
    void extractTestCase_shouldReturnTestCase_whenAgentSucceeds() {
        ...
        AgentExecutionResult<TestCase> executionResult = new AgentExecutionResult<>(
                AgentExecutionResult.ExecutionStatus.SUCCESS, "Success", true, expectedTestCase, java.time.Instant.now()
        );
        ...
        try (MockedStatic<AiServices> aiServices = mockStatic(AiServices.class);
            ...
            // Execute
            Optional<TestCase> result = ApiTestAgent.extractTestCase(message);
    ...
    void extractTestCase_shouldReturnEmpty_whenAgentFails() {
        String message = "run test";
        AgentExecutionResult<TestCase> executionResult = new AgentExecutionResult<>(
                AgentExecutionResult.ExecutionStatus.ERROR, "Failed", false, null, java.time.Instant.now()
        );
    ...
    void executeTestCase_shouldPass_whenAllStepsSucceed() {
        ...
        AgentExecutionResult<EmptyExecutionResult> actionResult = new AgentExecutionResult<>(
                AgentExecutionResult.ExecutionStatus.SUCCESS, "Action Success", true, new EmptyExecutionResult(), java.time.Instant.now()
        );
        AgentExecutionResult<VerificationExecutionResult> verificationResult = new AgentExecutionResult<>(
                AgentExecutionResult.ExecutionStatus.SUCCESS, "Verification Success", true, new VerificationExecutionResult(true, "Passed"), java.time.Instant.now()
        );
        ...
        try (MockedStatic<AiServices> aiServices = mockStatic(AiServices.class);
            ...
            agentConfig.when(() -> AgentConfig.getTestCaseExtractionAgentModelProvider()).thenReturn(AgentConfig.ModelProvider.GOOGLE);
            agentConfig.when(() -> AgentConfig.getTestCaseExtractionAgentPromptVersion()).thenReturn("v1");
            // Add other config mocks as needed or rely on nice mocks if possible? No, strict stubbing usually.
            // But since I mock ModelFactory and PromptUtils, I might not need all config values if I mock their consumers correctly.
            ...
            agentConfig.when(AgentConfig::getTestStepVerificationAgentModelProvider).thenReturn(AgentConfig.ModelProvider.GOOGLE);
            ...
            when(mockStepActionAgentBuilder.systemMessageProvider(any())).thenReturn(mockStepActionAgentBuilder);
            when(mockStepActionAgentBuilder.tools(any(Object[].class))).thenReturn(mockStepActionAgentBuilder);
            when(mockStepActionAgentBuilder.toolExecutionErrorHandler(any())).thenReturn(mockStepActionAgentBuilder);
            ...
            // Verification
            aiServices.when(() -> AiServices.builder(ApiTestStepVerificationAgent.class)).thenReturn(mockStepVerificationAgentBuilder);
            when(mockStepVerificationAgentBuilder.chatModel(any())).thenReturn(mockStepVerificationAgentBuilder);
            when(mockStepVerificationAgentBuilder.systemMessageProvider(any())).thenReturn(mockStepVerificationAgentBuilder);
            when(mockStepVerificationAgentBuilder.tools(any(Object[].class))).thenReturn(mockStepVerificationAgentBuilder);
            when(mockStepVerificationAgentBuilder.toolExecutionErrorHandler(any())).thenReturn(mockStepVerificationAgentBuilder);
            ...
            // Verify
            assertThat(result.testExecutionStatus()).isEqualTo(TestExecutionResult.TestExecutionStatus.PASSED);
            assertThat(result.testCaseName()).isEqualTo("Test Case 1");
            assertThat(result.stepResults()).hasSize(1);
            assertThat(result.stepResults().get(0).executionStatus()).isEqualTo(TestStepResult.TestStepResultStatus.SUCCESS);
        }
    }
}