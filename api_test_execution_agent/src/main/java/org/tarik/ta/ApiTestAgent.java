...
public class ApiTestAgent {
    ...
    public static TestExecutionResult executeTestCase(String receivedMessage) {
        ...
        if (testCase == null) {
            return new TestExecutionResult("Unknown", TestExecutionResult.TestExecutionStatus.ERROR, List.of(), List.of(), now(), now(),
        ...
        try {
            ...
            var requestTools = new ApiRequestTools(apiContext);
            var assertionTools = new ApiAssertionTools(apiContext);
            var dataTools = new ApiDataTools(apiContext);
            ...
            if (testCase.preconditions() != null && !testCase.preconditions().isEmpty()) {
                ...
                if (hasPreconditionFailures(executionContext)) {
                    ...
                    return getFailedTestExecutionResult(executionContext, testExecutionStartTimestamp, failedPrecondition.errorMessage());
                }
            }
            ...
            if (hasStepFailures(executionContext)) {
                ...
                if (lastStep.executionStatus() == FAILURE) {
                    return getFailedTestExecutionResult(executionContext, testExecutionStartTimestamp, lastStep.errorMessage());
                } else {
                    return getTestExecutionResultWithError(executionContext, testExecutionStartTimestamp, lastStep.errorMessage());
                }
            } else {
    ...
    private static void executePreconditions(TestExecutionContext executionContext, ApiContext apiContext,
        ...
        if (preconditions != null && !preconditions.isEmpty()) {
            ...
            var preconditionVerificationAgent = getApiPreconditionVerificationAgent(apiContext, new RetryState());
            ...
            for (String precondition : preconditions) {
                ...
                var preconditionExecutionResult = preconditionActionAgent
                        .executeWithRetry(() -> preconditionActionAgent.execute(precondition, executionContext.getSharedData().toString(),
                                apiContext.toString()));
                resetToolCallUsage();

                if (!preconditionExecutionResult.isSuccess()) {
                    var errorMessage = "Failure while executing precondition '%s'. Root cause: %s".formatted(precondition,
                ...
                var verificationExecutionResult = preconditionVerificationAgent
                        .executeWithRetry(() -> preconditionVerificationAgent.verify(precondition,
                                        preconditionExecutionResult.getMessage(), lastResponseStatus, lastResponseBody,
                ...
                if (!verificationResult.success()) {
                    ...
                    executionContext.addPreconditionResult(new PreconditionResult(precondition, false, errorMessage,
    ...
    private static void executeTestSteps(TestExecutionContext executionContext, ApiContext apiContext, ApiRequestTools requestTools,
        ...
        for (TestStep testStep : executionContext.getTestCase().testSteps()) {
            ...
            try {
                ...
                var actionResult = testStepActionAgent.executeWithRetry(() -> {
                    testStepActionAgent.execute(actionInstruction, testData, executionContext.getSharedData().toString(),
                            !isUnattendedMode());
                    return null;
                });
                resetToolCallUsage();
                ...
                if (!actionResult.isSuccess()) {
                    var errorMessage =
                            "Error while executing action '%s'. Root cause: %s".formatted(actionInstruction, actionResult.getMessage());
                ...
                if (isNotBlank(verificationInstruction)) {
                    ...
                    var verificationExecutionResult = testStepVerificationAgent.executeWithRetry(
                            () -> testStepVerificationAgent.verify(verificationInstruction, actionInstruction, testData, lastResponseStatus,
                    ...
                    executionContext.addStepResult(
                            new TestStepResult(testStep, SUCCESS, null, actualResult, executionStartTimestamp, now()));
                } else {
                    executionContext.addStepResult(
                            new TestStepResult(testStep, SUCCESS, null, "No verification required", executionStartTimestamp, now()));
                }
            } catch (Exception e) {
    ...
    private static void addFailedTestStep(TestExecutionContext context, TestStep testStep, String errorMessage, String actualResult,
    ...
    private static ApiTestStepActionAgent getApiTestStepActionAgent(ApiRequestTools requestTools, ApiAssertionTools assertionTools,
                                                                    ApiDataTools dataTools, RetryState retryState) {
    ...
    private static ApiPreconditionVerificationAgent getApiPreconditionVerificationAgent(ApiContext apiContext,
                                                                                        RetryState retryState) {
        var model = getModel(getPreconditionVerificationAgentModelName(), getPreconditionVerificationAgentModelProvider());
        var prompt = loadSystemPrompt("api_precondition_verification", getPreconditionVerificationAgentPromptVersion(),
    ...
    private static ApiTestStepVerificationAgent getApiTestStepVerificationAgent(ApiContext apiContext,
        ...
        return builder(ApiTestStepVerificationAgent.class)
                ...
                .systemMessageProvider(_ -> prompt)
                .tools(new ApiAssertionTools(apiContext), new VerificationExecutionResult(false, ""))
                .toolExecutionErrorHandler(new DefaultErrorHandler(ApiTestStepVerificationAgent.RETRY_POLICY, retryState))
                .build();
    }
}