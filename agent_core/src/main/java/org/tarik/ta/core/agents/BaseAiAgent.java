...
import static org.tarik.ta.core.tools.AgentExecutionResult.ExecutionStatus.*;
...
public interface BaseAiAgent<T extends FinalResult<T>> {
    ...
    default AgentExecutionResult<T> executeAndGetResult(Supplier<Result<?>> action) {
        ...
        try {
            ...
            return new AgentExecutionResult<>(ERROR, e.getMessage(), false, captureErrorScreenshot(), null, now());
        }
    }
            ...
            try {
                ...
                switch (e) {
                    case ToolExecutionException tee when tee.getErrorCategory() == TERMINATION_BY_USER -> {
                        ...
                        return new AgentExecutionResult<>(INTERRUPTED_BY_USER, e.getMessage(), false, captureErrorScreenshot(), null, now());
                    }
                    case ToolExecutionException tee when tee.getErrorCategory() == VERIFICATION_FAILED -> {
                        return new AgentExecutionResult<>(VERIFICATION_FAILURE, e.getMessage(), false, captureErrorScreenshot(), null, now());
                    }
    ...
    private AgentExecutionResult<T> handleRetry(int attempt, long startTime, RetryPolicy policy, String message,
        ...
        if (isTimeout || isMaxRetriesReached) {
            LOG.error("Operation for task '{}' failed after {} attempts (elapsed: {}ms). Last error: {}", taskDescription, attempt,
                    elapsedTime, message);
            return new AgentExecutionResult<>(ERROR, message, false, captureErrorScreenshot(), null, now());
        }
